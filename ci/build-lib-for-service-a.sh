...
mvn -s git-repo-all-out/ci/settings.xml -f git-repo-all-out/lib-for-service-a/pom.xml versions:set -DnewVersion=$NEW_VERSION -DgenerateBackupPoms=false -DoldVersion='*'
echo "Building lib-for-service-a module..."
mvn -s git-repo-all-out/ci/settings.xml -f git-repo-all-out/lib-for-service-a/pom.xml clean install
echo "Moving jar to output folder..."
mv git-repo-all-out/lib-for-service-a/target/*.jar global-output/
echo "Cleaning..."
mvn -s git-repo-all-out/ci/settings.xml -f git-repo-all-out/lib-for-service-a/pom.xml clean
